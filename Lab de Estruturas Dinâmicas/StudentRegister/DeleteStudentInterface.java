...
public class DeleteStudentInterface extends JFrame
{
	...
	{//interface para deletar dados de estudante do registro
		int n = delete.length;
		assert n == displayInfo[0].length && n == displayInfo[1].length :
			new Exception("The lengths of the boolean array delete and the String arrays displayInfo[0] and displayInfo[1] do not match.");
		...
		scrollPane = new JScrollPane(deletePanel, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		
		contentPane.add(confirmPanel);
		contentPane.add(scrollPane);
		contentLayout.putConstraint(SpringLayout.NORTH, confirmPanel, 0, SpringLayout.NORTH, contentPane);
		sideConstraint(confirmPanel, contentPane, contentLayout);
		contentLayout.putConstraint(SpringLayout.NORTH, scrollPane, 0, SpringLayout.SOUTH, confirmPanel);
		contentLayout.putConstraint(SpringLayout.SOUTH, scrollPane, 0, SpringLayout.SOUTH, contentPane);
