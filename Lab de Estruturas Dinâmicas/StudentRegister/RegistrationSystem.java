...
{//Usando ActionListener para registrar ActionEvents como os componente do JButton e  WindowListener para receber eventos de novas janelas
	...
	private void saveFile()
	{
		if (currentFile != null && currentFile.exists()){
			try {
				ObjectOutputStream output =	new ObjectOutputStream(new FileOutputStream(currentFile));
				int i = 0, n = getNumberOfStudents();
				try {
					for (i = 0; i < n; i++)
						output.writeObject(students[i]);
					JOptionPane.showMessageDialog(null, i + " student records have been saved to the file " + currentFile);
				} finally {
					output.close();
				} 
			} catch (IOException e) {
	...
	{
		boolean isOpenSpace = (students[students.length-1] == null || !students[students.length-1].exists());
		if (isOpenSpace){
	...
	private void siftIDDown(int root, int bottom) 
	{
		...
		while (root * 2 <= bottom && !done) {
			...
			boolean leftChildIsLarger = ( getStudentID(root * 2) > getStudentID((root * 2) + 1) );
