...
public class CourseListingSystem
{
	...
	{//encontrar curso salvo no array, procurando por palavras chaves e retornando dados parciais
		...
		try {
			inputString = JOptionPane.showInputDialog(null,	"Please enter all or part of the course name, course number or instructor name to select a course.", null);
			do {
				numberOfMatches = 0;
				results = "";
				for (int i = 0; i <= 11; i++) {
					if (matchesInput(courseArray[i], inputString)) {
						...
						if (numberOfMatches < MAX_RESULTS_DISPLAYED)
								results = results.concat(courseArray[i].getCourseName() + ", " + courseArray[i].getCourseNumber() + " taught by " 
	...
	{//array com vetores de cursos salvos
